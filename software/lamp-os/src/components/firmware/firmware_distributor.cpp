...
// never have been Serial.begin()'d) don't block on UART. Per the
// 2026-06-10 code review, all the diagnostic prints in this file
// route through these macros — they collapse to no-op on host builds
// (no Serial) and on release builds (no LAMP_DEBUG).
...
// constant because the on-wire field is just a uint16_t the receiver
// trusts at face value; we hardcode the v1 value here (matching the
// wisp distributor + firmware_signature kLsigFooterLen).
...
void FirmwareDistributor::begin(FirmwareTransport* transport) {
  ...
  // partition capacity. Per the 2026-06-10 code review (bug #5 — the
  // showstopper): the running partition is sized in partitions.csv to be
  // larger than the image (~1.5 MB partition, ~1.35 MB signed image),
  // with the tail erased to 0xFF. Sending `partition->size` bytes would
  // stream 1.4 MB of mostly-erased flash garbage, and the SHA wouldn't
  // match what the receiver computes over its own newly-written image.
  //
  ...
  while (scanEnd > 0) {
    ...
    for (size_t i = take; i > 0; --i) {
      ...
      if (buf[off]     == kMagic[0] &&
        ...
        // at footer offset 16 (4 bytes LE).
        const uint32_t footerStart = scanStart + off;
        uint8_t lenBytes[4];
        if (!readPartitionBytes(footerStart + 16, 4, lenBytes)) return false;
...
void FirmwareDistributor::considerPeerForOta(const uint8_t peerMac[6],
  ...
  if (peerIsInBackoff(peerMac, nowMs)) return;
...
void FirmwareDistributor::emitOffer(const uint8_t targetMac[6],
  ...
  //
  // Per 2026-06-10 code review bug #4: re-check `state_ == Idle` INSIDE
...
bool FirmwareDistributor::sendOfferFrame(const uint8_t targetMac[6],
  ...
  // Per 2026-06-10 code review bug #7: bump lastOfferSendMs_ regardless
...
void FirmwareDistributor::onReq(const lamp_protocol::ParsedFwReq& r,
  ...
  // Rewind cursor. Streaming task re-emits from here; once it walks past
  // the gap it naturally resumes forward emit.
