...
//
// Phase 5b' (gossip-OTA spike): mirror of the wisp's FirmwareDistributor
// (software/wisp/src/FirmwareDistributor.h) with the venue-specific
// adaptations spelled out below. The shared shape (state machine, peer
// backoff ring, per-chunk retries, OFFER retries, FINALIZE timeout, DONE
// retry, streaming task) is intentionally identical — those tunables were
// hardware-tuned for ESP-NOW under BLE coex and carry over unchanged. The
...
//        standard_lamp.cpp will gate considerPeerForOta on
...
class FirmwareDistributor {
 public:
  ...
  // Per-session REQ budget. A healthy session for a ~1.5 MB image (7500
  // chunks @ 200 B) needs at most a handful of REQs even under heavy
  // packet loss — single-digit. 32 leaves ample headroom for legitimate
  // jitter while bounding the "REQ-flood rewinds stream forever" attack
  // surface from a buggy or hostile receiver. Hitting the cap aborts
  // the session with a 10-minute backoff for that peer.
  static constexpr uint16_t kMaxReqPerSession   = 32;
 ...
 private:
  ...
  // of the signed image. Per 2026-06-10 code review bug #5 — the
  // partition is larger than the image; sending partition->size would
  // stream erased flash garbage. Sets *outLen to the position just past
  // the last non-0xFF byte (i.e., signed-image-length including the
  // LSIG footer). Returns false on partition read failure.
  ...
  // Per-session REQ counter (Phase 5b' hardening). Bounded by
  // kMaxReqPerSession; exceeding aborts the session with peer backoff.
...
// Single global instance, defined in standard_lamp.cpp. SocialBehavior +
