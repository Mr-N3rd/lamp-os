...
// wrong sizes.
static_assert(lp::FW_OFFER_FIXED_SIZE  == 48, "FW OFFER size pin");
...
static_assert(lp::FW_CHANNEL_LEN       ==   8, "FW channel slot lock");
...
void test_fw_offer_roundtrip() {
  ...
  const char channel[] = "stable";   // 6 bytes; builder zero-pads to 8
  ...
  TEST_ASSERT_EQUAL_STRING("stable", out.channel);
...
void test_fw_offer_full_channel_string() {
  // Exactly 8 bytes of channel — no terminator on the wire, but the parsed
  // struct gets a null-terminator appended at index 8.
  uint8_t buf[lp::FW_OFFER_FIXED_SIZE];
  const char channel[8] = {'a','b','c','d','e','f','g','h'};
  ...
  lp::ParsedFwOffer out;
  TEST_ASSERT_TRUE(lp::parseFwOffer(buf, n, out));
  TEST_ASSERT_EQUAL_STRING("abcdefgh", out.channel);
  TEST_ASSERT_EQUAL_CHAR('\0', out.channel[8]);
}
