...
// no ACK received within retries. Without this hook a silently-dropped
// unicast looks like ESP_OK at the call site (the IDF's enqueue), which
// previously made OFFER/ACCEPT drops invisible. We log FAILs in
// LAMP_DEBUG builds and maintain a running counter the distributor can
// query (sendFailCount_) so future retry logic can drive off observed
// failure instead of timer-based guessing.
...
static void sendTrampoline(const wifi_tx_info_t* info,
  ...
  if (status == ESP_NOW_SEND_SUCCESS) {
    MeshLink::s_instance->noteSendResult(mac, /*ok=*/true);
    return;
  }
  MeshLink::s_instance->noteSendResult(mac, /*ok=*/false);
...
void MeshLink::noteSendResult(const uint8_t* /*mac*/, bool ok) {
  if (ok) {
    sendOkCount_++;
  } else {
    sendFailCount_++;
