...
/// Header constants — must match `LM` magic + `PROTOCOL_VERSION = 0x03`
/// in the C++ side. Bumping the version here without bumping the lamp's
/// is an immediate mesh-fleet incompatibility (loud, diagnosable: peers
/// just don't show up).
const int magic0 = 0x4C; // 'L'
const int magic1 = 0x4D; // 'M'
const int protocolVersion = 0x03;
...
/// Zero-padded ASCII channel string width.
const int fwChannelLen = 8;
...
Uint8List buildFwOffer({
  ...
  view.setUint16(26, chunkSize, Endian.little);
  // Channel: zero-pad to 8 bytes, truncate if longer.
  final channelBytes = channel.codeUnits;
  for (var i = 0; i < fwChannelLen; ++i) {
    view.setUint8(28 + i, i < channelBytes.length ? channelBytes[i] : 0);
  }
  for (var i = 0; i < fwSha256PrefixLen; ++i) {
    view.setUint8(36 + i, sha256Prefix[i]);
  }
  view.setUint16(44, footerLen, Endian.little);
  view.setUint16(46, totalChunks, Endian.little);
  return bytes;
}
