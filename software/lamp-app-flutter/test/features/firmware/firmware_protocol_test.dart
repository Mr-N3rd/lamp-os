...
void main() {
  group('buildFwOffer', () {
    test('emits a 48-byte frame with correct header + LE encoding', () {
      ...
      // Channel — zero-padded to 8
      expect(frame.sublist(28, 36),
          equals([0x73, 0x74, 0x61, 0x62, 0x6C, 0x65, 0x00, 0x00]));
      // SHA-256 prefix
      expect(frame.sublist(36, 44), equals(_sha8()));
      // Footer len (LE)
      final f = ByteData.view(frame.buffer, frame.offsetInBytes + 44, 2);
      expect(f.getUint16(0, Endian.little), equals(96));
      // Total chunks (LE)
      final tc = ByteData.view(frame.buffer, frame.offsetInBytes + 46, 2);
      expect(tc.getUint16(0, Endian.little), equals(5243));
    });

    test('truncates a >8-char channel', () {
      final frame = buildFwOffer(
        seq: 1,
        sourceMac: _macAB(),
        targetMac: _macFF(),
        ...
        totalLen: 100000,
        chunkSize: 200,
        channel: 'extra-long-channel-name',
        sha256Prefix: _sha8(),
        footerLen: 96,
        totalChunks: 500,
      );
      // Only first 8 bytes copied; not null-terminated on the wire.
      expect(frame.sublist(28, 36),
          equals('extra-lo'.codeUnits));
    });
