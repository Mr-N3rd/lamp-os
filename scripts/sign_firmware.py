...
def _parse_firmware_version() -> tuple[int, str]:
    """Return (packed_u32, human_str). Single source of truth: version.hpp."""
    # Preferred path: defines header at include/firmware_version.h (per spec).
    ...
    # Fallback: src/version.hpp's `FIRMWARE_VERSION = 0xMMmmpp` literal.
...
def _resolve_channel() -> bytes:
    raw = os.environ.get("LAMP_FIRMWARE_CHANNEL", DEFAULT_CHANNEL)
    encoded = raw.encode("ascii", errors="strict")
    if len(encoded) > LSIG_CHANNEL_LEN:
        print(
            f"[sign] FATAL: LAMP_FIRMWARE_CHANNEL={raw!r} exceeds {LSIG_CHANNEL_LEN} "
...
def _build_footer(
    ...
    footer[4:12] = channel
    struct.pack_into("<I", footer, 12, fw_version & 0xFFFFFFFF)
    struct.pack_into("<I", footer, 16, signed_region_len & 0xFFFFFFFF)
    # bytes [20..32) already zeros from bytearray() — reserved
    footer[32:96] = signature
    assert len(footer) == LSIG_FOOTER_LEN
    return bytes(footer)


def _is_up_to_date(input_bin: Path, signed_bin: Path, private_key: Path) -> bool:
    """True iff signed_bin exists and is newer than the input + key."""
    if not signed_bin.exists():
        return False
    signed_mtime = signed_bin.stat().st_mtime
    if input_bin.exists() and input_bin.stat().st_mtime > signed_mtime:
        return False
    if private_key.exists() and private_key.stat().st_mtime > signed_mtime:
        return False
    # Also re-sign if the channel env var changed mid-session: parse the
    # existing footer and compare. Cheap because we only read the last
    # 96 bytes.
    try:
        with open(signed_bin, "rb") as fh:
            fh.seek(-LSIG_FOOTER_LEN, os.SEEK_END)
            footer = fh.read(LSIG_FOOTER_LEN)
        if footer[0:4] != LSIG_MAGIC:
            return False
        if footer[4 : 4 + LSIG_CHANNEL_LEN] != _resolve_channel():
            return False
        expected_version, _ = _parse_firmware_version()
        embedded_version = struct.unpack_from("<I", footer, 12)[0]
...
def sign(input_bin: Path, output_bin: Path) -> None:
