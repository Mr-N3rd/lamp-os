...
This doc is for developers modifying the lamp's expressions subsystem or adding a new expression type. For end-user UX, see the app's Expressions tab.
...
2. **Factory arm** in `expression_manager.cpp::makeExpression()`. Add `if (type == "foo") return std::make_unique<FooExpression>(...);`. Call `configure(colors, intervalMin, intervalMax, target)` then `configureFromParameters(parameters)` if your subclass overrides those.
